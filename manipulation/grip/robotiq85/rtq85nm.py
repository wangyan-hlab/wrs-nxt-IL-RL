...
from utiltools import designpattern
...
class Rtq85NM():
    ...
    def gripAt(self, fcx, fcy, fcz, c0nx, c0ny, c0nz, rotangle = 0, jawwidth = 82):
        ...
        handmat4 = self.rtq85np.getMat()
        self.rtq85np.setMat(handmat4*Mat4.rotateMat(90.0, handmat4.getRow3(2)))
...
def newHandNM(hndid = 'rgt', jawwidth = 85, hndcolor = None, ftsensoroffset = 52):
    return Rtq85NM(jawwidth, hndcolor, ftsensoroffset)
...
if __name__=='__main__':
    ...
    rtq85hnd = Rtq85NM(hndcolor=[.5,.5,0.5,.7], ftsensoroffset=0)
    rtq85hnd.setJawwidth(70)
    rtq85hnd.gripAt(0,0,0,0,0,1,30)
    rtq85hnd.reparentTo(base.render)
    rtq85hnd2 = Rtq85NM(ftsensoroffset=0)
    rtq85hnd2.setJawwidth(70)
    rtq85hnd2.gripAt(0,50,0,0,0,1,30)
    ...
    #     cp = contact.getManifoldPoint()
    #     print(cp.getLocalPointA())
    #     pgg.plotSphere(base, pos=cp.getLocalPointA(), radius=10, rgba=Vec4(1,0,0,1))
    #
    #     pgg.plotAxis(base.render, spos = Vec3(0,0,0))
    #
    # debugNode = BulletDebugNode('Debug')
    # debugNode.showWireframe(True)
    # debugNode.showConstraints(True)
