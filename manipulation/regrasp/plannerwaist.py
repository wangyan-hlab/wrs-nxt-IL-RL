                ...
                if i > 0:
                    ...
                    if (startid[:-1] == endid[:-1]):
                        if (startid[-1] != "i") and (endid[-1] != "i"):
                            ...
                            tempobjmsmp = []
                            temparmname = "rgt"
                            startjntags = [numikrms[i-1][0], numikrms[i-1][1]]
                            goaljntags = [numikrms[i][0], numikrms[i][1]]
                            if "lft" in startid:
                                temparmname = "lft"
                                startjntags = [numikrms[i-1][0], numikrms[i-1][2]]
                            ...
                            for eachitem in interplatedjnts:
                                if temparmname == "rgt":
                                    tempnumikrmsmp.append([eachitem[0], eachitem[1], numikrms[i-1][2]])
                                else:
                                    tempnumikrmsmp.append([eachitem[0], numikrms[i-1][1], eachitem[1]])
                            ...
                            # jawwidthmp.append([jawwidth[i-1], jawwidth[i]])
                            # continue
                    if (startid[0] not in ['b', 's', 'g']) and (endid[0] not in ['b', 's', 'g']):
                        print("Planning ho motion between ", pathnidlist[i-1], " and ", pathnidlist[i])
                    ...
                    if "lft" in endid:
                        armname = 'lft'
                        start = [numikrms[i-1][0]]+numikrms[i-1][2].tolist()
                    ...
                    if (endid[-1] == "c") or (endid[-1] == "w"):
                        ...
                        for j in range(npath):
                            if armname == 'rgt':
                                tempnumikrmsmp.append([path[j][0], np.array(path[j][1:]), numikrms[i-1][2]])
                            else:
                                tempnumikrmsmp.append([path[j][0], numikrms[i-1][1], np.array(path[j][1:])])
                            robot.movearmfkr([path[j][0], np.array(path[j][1:])], armname = armname)
                            tempjawwidthmp.append(jawwidth[i-1])
                            objpos, objrot = robot.getworldpose(relpos, relrot, armname)
                            tempobjmsmp.append(base.pg.npToMat4(objrot, objpos))
                    else:
