...
class FreeTabletopPlacement(object):
    ...
    def __init__(self, objpath, handpkg, gdb):
        ...
        self.ocfacets, self.ocfacetnormals, curvature = self.objtrimeshconv.facets_over(.9999)
        ...
        self.planebullnode = cd.genBulletCDPlane(offset=0)
    ...
    def __genTps(self, base, doverh=.1):
        """
        generate free tabletopplacements by
        ...
        for i in range(len(self.ocfacets)):
            ...
            geombullnode = cd.genBulletCDMeshGeom(geom)
            ...
            if result.hasHit():
                ...
                bdverts3d, bdverts2d, facetnpmat4 = base.pg.facetboundary(self.objtrimeshconv, self.ocfacets[i],
                                                                     facetinterpnt, facetnormal)
                facetp = Polygon(bdverts2d)
                facetinterpnt2d = rm.homotransform(facetnpmat4, facetinterpnt)[:2]
                ...
                if dist2p/dist2c >= doverh:
                    # hit and stable
                    self.tpsmat4s.append(base.pg.np4ToMat4(facetnpmat4))

    def __genTpsGrips(self, base):
        """
    ...
    def genTpsandGrips(self, base, doverh=.1):
        """
        a helper function that does genTps and gen Tps Grips at the same time
        ...
        self.__genTps(base, doverh)
        self.__genTpsGrips(base)
        ...
        if len(result) != 0:
            ...
            if isredo != "Y" and isredo != "y":
                ...
                for eachresult in result:
                    sql = "DELETE FROM freetabletopgrip WHERE freetabletopgrip.idfreetabletopplacement LIKE '%s'" % \
                          eachresult[0]
                    self.gdb.execute(sql)
                for eachresult in result:
                    sql = "DELETE FROM freetabletopplacement WHERE freetabletopplacement.idfreetabletopplacement LIKE '%s'" % \
                          eachresult[0]
                    self.gdb.execute(sql)
    ...
    def showOnePlacementAndAssociatedGrips(self, base, id):
        """
        ...
        for i in range(len(self.tpsmat4s)):
            if i == id:
                ...
                # show object
                geom = base.pg.packpandageom_fn(self.objtrimesh.vertices,
                                        self.objtrimesh.face_normals,
                                        self.objtrimesh.faces)
                node = GeomNode('obj')
                node.addGeom(geom)
    ...
    def ocfacetshow(self, base):
        ...
        if self.counter < len(self.ocfacets):
            geom = base.pg.packpandageom(self.objtrimeshconv.vertices+
...
if __name__ == '__main__':
    ...
    objpath = os.path.join(os.path.split(this_dir)[0]+os.sep, "grip", "objects", "bunnysim.stl")
