...
if __name__ == '__main__':
    ...
    inittcppos, inittcprot = ur5dualrobot.gettcp(armname)

    def updateshow(ur5dualrobot, dfvalue, armname, robotnp, arrownp, task):
        ...
        armjntsgoal = u5dik.numik(ur5dualrobot, tgtpos, tgtrot, armname)
        if armjntsgoal is not None:
            ur5dualrobot.movearmfk(armjntsgoal, armname)

        return task.again

    taskMgr.add(updateshow, 'updateshow', extraArgs = [ur5dualrobot, dfvalue, armname, robotnp, arrownp], appendTask = True)
    # taskMgr.doMethodLater(10, updateshow, 'updateshow', extraArgs = [ur5dualrobot, dfvalue, armname, robotnp, arrownp], appendTask = True)
