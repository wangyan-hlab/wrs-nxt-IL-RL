...
class Ur5DualUrx():
    ...
    def movejntssgl(self, joints, armname='rgt'):
        """
    ...
    def getforcevec(self, armname='rgt'):
        """
...
if __name__ == '__main__':
    ...
    # armname = 'lft'
    # # ur5u.movejntssgl(ur5dualrobot.initjnts[9:15], armname)
    # ur5dualrobot.movearmfk(ur5dualrobot.initjnts[9:15], armname)
    # tcprobot =  ur5u.gettcp(armname)
    ...
    def updateshow(ur5u, ur5dualrobot, armname, robotnp, arrownp, counter, fvalhandinit, tcpinit, tpose, task):
        ...
        force =  ur5u.getforcevec(armname = armname)
        basemat4 = Mat4.rotateMat(-145, Vec3(1,0,0))
        tcpmat4 = ur5dualrobot.gettcpframe(armname)
        ...
        if counter[0] == 0:
            ...
            tcpinit[0], tcpinit[1] = ur5dualrobot.gettcp(armname)
            counter[0] += 1
            task.again
        ...
        armjntsgoal = u5dik.numik(ur5dualrobot, tcpgoalpos, tcpgoalrot, armname)
        if armjntsgoal is not None:
            ur5dualrobot.movearmfk(armjntsgoal, armname)
            # ur5u.movejntssgl(armjntsgoal.tolist(), armname)

        return task.again

    armname = 'lft'
    taskMgr.add(updateshow, 'updateshow', extraArgs = [ur5u, ur5dualrobot, armname, robotnp, arrownp, counter, fvalhandinit, tcpinit, tpose], appendTask = True)
