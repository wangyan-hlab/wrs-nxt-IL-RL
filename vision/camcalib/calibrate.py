...
import vision.camcalib.utiltools as cu
...
def calibchessboard(ncrossrow, ncrosscolumn, markersize = 25, imgspath='./', savename='mycam_data.yaml'):
    ...
    if ret:
        with open(savename, "w") as f:
            yaml.dump([mtx, dist, rvecs, tvecs], f)

def calibcharucoboard(nrow, ncolumn, arucomarkerdict=aruco.DICT_6X6_250, squaremarkersize=25, imgspath='./', savename='mycam_charuco_data.yaml'):
    """
...
def find_rhomo(basecamyamlpath, relcamyamlpath, savename):
    ...
    mtx_b, dist_b, rvecs_b, tvecs_b, candfiles_b = yaml.load(open(basecamyamlpath, 'r'))
    mtx_r, dist_r, rvecs_r, tvecs_r, candfiles_r = yaml.load(open(relcamyamlpath, 'r'))
...
if __name__=='__main__':
    ...
    # show in videos
    mtx0, dist0, rvecs0, tvecs0, candfiles0 = yaml.load(open('cam0_calib.yaml', 'r'))
    mtx2, dist2, rvecs2, tvecs2, candfiles2 = yaml.load(open('cam2_calib.yaml', 'r'))
    mtx4, dist4, rvecs4, tvecs4, candfiles4 = yaml.load(open('cam4_calib.yaml', 'r'))
