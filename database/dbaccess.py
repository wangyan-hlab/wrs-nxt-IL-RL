...
class GraspDB(object):
    ...
    def __init__(self, ip = "localhost", user = "root", password = "lovemaomao1123", database = "grip"):
        self.dbconnection = mdb.connect(host = ip, user = user, passwd = password, db = database, port = 3306)
    ...
    def loadIKRet(self, worlda=Vec3(0,0,1)):
        sql = "SELECT rethanda, retworlda FROM ikret"
        result = self.execute(sql)
        if len(result) != 0:
            rethanda = result[0][0]
            retworlda = result[0][1]
        else:
            # default value: 50,50,50
            sql = "INSERT INTO ikret VALUES (70,70)"
            self.execute(sql)
            rethanda = 70
            retworlda = 70
        return [rethanda, retworlda, worlda]
    ...
    def loadIdHand(self, handname):
        ...
        if len(result) != 0:
            ...
            sys.exit("No hand found in hand table!")
        return idhand
