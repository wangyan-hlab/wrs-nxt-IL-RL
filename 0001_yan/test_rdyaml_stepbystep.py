...
if __name__ == "__main__":
    ...
    # nxtu = nxturx.NxtCon()
    # nxtu.setJointAnglesOfGroup('rarm', nxtrobot.initjnts[3:9], 5.0, True)
    # nxtu.setJointAnglesOfGroup('larm', nxtrobot.initjnts[9:15], 5.0, True)
    ...
    for i in range(num):
        ...
        print("The newpath is %s" % newpath)
    ...
    def updateshow(rbtmnp, motioncounter, robot, path, armname, robotmesh, robotball, task):
        if motioncounter[0] < len(path):
            ...
            rbtmnp[0] = robotmesh.genmnp(robot, jawwidthrgt=45, jawwidthlft=0)
    ...
    def updatesection(newpath, motioncounter, robot, robotmesh, robotball, task):
            ...
            if counter[0] < len(newpath):
                ...
                if rgtdiff.max() > 1e-6:
                    ...
                    for i in range(len(path)):
                        ...
                        path_rad.append(pathpt_rad[len(path[i]) * i:len(path[i]) * (i + 1)])
                    print("path_rad is %s" % path_rad)
                    # time = [0.2]
                    # para_rgt = ['rarm', path_rad, time * len(path_rad)]
                    # nxtu.playPatternOfGroup(para_rgt)
