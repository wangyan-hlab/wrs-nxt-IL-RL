...
def motionplanning(start, goal, fixedobj, maniobj, expdis):
    planner = ddrrtc.DDRRTConnect(start=start, goal=goal, ctcallback=ctcallback,
                                  ...
                                  expanddis=expdis, maxiter=1000, maxtime=400)
    obscmlist = [fixedobj]
...
if __name__=="__main__":
    ...
    start = np.array([virtualgoalpos1[0], virtualgoalpos1[1], virtualgoalpos1[2],
                      virtualgoalrot1[0], 0, 0], dtype=float)
    ...
    # start:
    # demopathlist = [np.array([ -5.73826664, -79.25563482,  14.92496054]),
    #                 np.array([[ 0.9981996 ,  0.0533105 , -0.0274875 ],
    ...
    wholepath = []
    for pose in demopathlistxyzRPY:
        wholepath.append([pose[0], pose[1], pose[2], pose[3], pose[4], pose[5]])
    # wholepath.append([0.0, -78.17081451416016, 28.519912719726562, -28.36366915896073, 0.0, 0.0])

    wholepath = [[  0.        , -77.27624512,  13.92599487, -18.06810583,
         0.        ,   0.        ], [  0.        , -69.37454987,   4.84276724,  -8.20684612,
         0.        ,   0.        ], [  0.        , -69.7228241 ,   3.149616  ,  -6.32433153,
         0.        ,   0.        ]]
    num = 0
    for pos in wholepath:
        ...
        tmpobjcm.setColor(1, 0, 0, 1-num*0.2)
