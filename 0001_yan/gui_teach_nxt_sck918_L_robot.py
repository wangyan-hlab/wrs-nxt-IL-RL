...
class NxtDTCtl(object):
    ...
    def __init__(self, objname, markerid = 5):
        ...
        # self.nxtu.playPattern([anglesrad], [5.0])
    ...
    def rgtcapture(self):
        ...
        goalTlist = []
        # plannedpath = readfile("Planned_Lpath_deriv1.txt")
        plannedpath = readfile("Planned_Lpath_deriv2.txt")
        ...
        if goalT is None:
            ...
            for id, tmpobjcm in enumerate(self.objrenderlist):
                ...
                tmpobjcm.setColor(1, 1, 0, 1 - id *0.005)
            return True
    ...
    def rgtcapturestart(self):
        ...
        virtualgoalpos0 = np.array([450,-200,973-76+75+100])
    ...
    def plannplay(self, transitoption = "useboth", choice = "startrgtgoalrgt"):
        ...
        def updatesection(objmsmpactive, numikrmsmpactive, jawwidthmpactive, objmsmp, numikrmsmp, jawwidthmp, rbtmnp,
            ...
            if base.inputmgr.keyMap['space'] is True and (motionseccounter[0] <= 8 and not taskMgr.hasTaskNamed("updatemotionsec")):
                        ...
                        if len(jntslist_rgt) == 4:
                            ...
                            for n in range(length):
                                ...
                                if jntslist_lft_rad:
                                    ...
                                    leftangles = [math.radians(15), 0, math.radians(-143), 0, 0, 0]
                                    for leftangle in leftangles:
                                        angle.append(leftangle)
                            ...
                            for n in range(len(angles)):
                                time.append(1)
                            self.nxtu.playPattern(angles, time)
            ...
            if base.inputmgr.keyMap['space'] is True or (motionseccounter[0] > 8 and not taskMgr.hasTaskNamed("updatemotionsec")):
                # print(motionseccounter[0], len(objmsmp) - 1)
                if motionseccounter[0] <= len(objmsmp) - 1:
                    motionseccounter[0] = motionseccounter[0] + 1
                    if not (motionseccounter[0] == len(objmsmp)):
...
if __name__=="__main__":
    base = pc.World(camp=[400,2000,1000], lookatp=[400,200,1000], up = [0,0,1], fov = 40, w = 1920, h = 1080)
    # objname = "housing.stl"
    # objname = "box.stl"
